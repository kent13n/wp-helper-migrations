...
class ContentListTable extends \WP_List_Table
{
        ...
        switch ($column_name) {
            case 'date':
                ...
                if ($item['type'] === 'nav_menu') {
                    $t_time = date('Y/m/d g:i:s a', $item['date']);
                    $time = $item['date'];
                    $time_diff = time() - $time;
                } else {
     ...
     * @return void
     */
    public function prepare_nav_items()
    {
        $columns = $this->get_columns();
        $sortable = $this->get_sortable_columns();
        $hidden = [];
        ...
        /*$this->process_nav_migration();
        $this->process_nav_bulk_action();*/
        ...
        if (count($menus) > 0) {
            foreach ($menus as $menu) {
                ...
                foreach ($menu_items as $item) {
                    if ($date === null || strtotime($date) < strtotime($item->post_modified)) {
