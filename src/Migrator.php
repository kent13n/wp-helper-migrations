...
class Migrator {
	...
	private static function ExtractMode( array $post_meta, Enums\Mode $mode ): Enums\Mode {
		...
		} else if ( $mode === Enums\Mode::Unknown ) {
			$result = Enums\Mode::Insert;
		}
	...
	public static function Generate( int $post_id, Enums\Mode $mode = Enums\Mode::Unknown ): bool {
		...
		$success                       = MigrationHandler::CreateMigrationFile( self::$wp_helper_migrations_id, $sql );
		self::$wp_helper_migrations_id = '';
	...
	public static function Run(): int {
		...
		foreach ( $migrations as $m ) {
			...
			$migration->Run();
			$wpdb->insert( 'wp_helper_migrations', array(
				'name'       => $className,
